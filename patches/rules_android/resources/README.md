...
- `skip_instrumentation_min_sdk_validation.patch` — keep that instrumentation manifest parity after `min_sdk_version` is forwarded to native.android_binary for desugar/dex. rules_android validates that the processed manifest contains the processed min SDK; instrumentation APKs intentionally do not stamp `<uses-sdk>`, so validation is skipped only for `android_binary(instruments = ...)`.
