...
- `skip_binary_r_jar_desugaring_flags.patch` — adds a transitional `//rules/flags:desugar_resources_jar` bool_flag for compatibility.
- `skip_binary_r_jar_desugaring_attrs.patch` — wires the transitional flag as a private attr on android_binary.
- `skip_binary_r_jar_desugaring_impl.patch` — in android_binary impl, when the native `--experimental_android_desugar_resources_jar=false` flag or the transitional Starlark flag disables resource-jar desugaring and the jar is `_resources.jar`, pass through untransformed.
...
Flip `common --experimental_android_desugar_resources_jar=false` in bazelrc to enable the binary-side skip. The transitional `--@rules_android//rules/flags:desugar_resources_jar=false` surface is still honored while repos migrate back to the Bazel 7-style native flag spelling.
